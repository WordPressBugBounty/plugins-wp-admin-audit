...
class WADA_Settings
{
    ...
    public static function getDatabaseVersion($default=null, $storeInDbIfNotExists=false){
        return self::getSetting(self::DB_VERSION, $default ?: '1.0', $storeInDbIfNotExists, false);
    }
    ...
    public static function getLoggingLevel($default=WADA_Constants::LOG_LEVEL_INFO) {
        $level = self::getSetting(self::LOGGING_LEVEL, $default, false, false);
        return intval($level);
    }
    ...
    public static function isLoggingForced($default=0) {
        $isForced = self::getSetting(self::LOGGING_FORCED, $default, false, false);
        return (intval($isForced) > 0);
    }
    ...
    protected static function getSetting($settingId, $default=null, $storeInDbIfNotExists=false, $prepareDatatypeAccordingtoMetadata=true){
        if((strlen($settingId) > 7) && substr($settingId, 0, strlen('setting')) === 'setting'){
        ...
        if($prepareDatatypeAccordingtoMetadata){
            $metaData = self::getMetadataForSetting($settingId);
            if($metaData){
